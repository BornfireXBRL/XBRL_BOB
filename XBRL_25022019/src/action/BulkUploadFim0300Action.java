...
public class BulkUploadFim0300Action 
{
	static Logger log = Logger.getLogger(BulkUploadFim0300Action.class);
	...
	public String fim0300Upload() throws SQLException, IOException 
	{
		log.info("xcel fn ");
		...
		{
			sql1.append("TRUNCATE  TABLE FIM0300_MANUAL_TABLE");
					          ...
					          .open(is)) 
			{
					  ...
					  for (Sheet s : workbook)
					  {
					    ...
					    for (Row r : s) 
					    {
							...
							if (i1 > 0) {
								...
								log.info(dataHolder);
								String query = "INSERT INTO FIM0300_MANUAL_TABLE(COUNTRY_CODE,CURRENCY_CODE,UNITS,TTB_RATE,TCB_RATE,NTB_RATE,TTS_RATE,NTS_RATE,DEL_FLG,RCRE_USER_ID, RCRE_TIME,REPORT_DATE) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
	            ...
	            log.info("inside the catch");
					...
					if (rs > 0) 
					{
						...
						pstmt.setString(1, "FIM0300_MANUAL");
						...
						status = "success";
						request.setAttribute("Page", "bulkuploadfim300");
	...
	public String fim0300Upload1() throws SQLException, IOException 
	{
		...
		for (int i = 0; i < fileUpload.length; i++) 
		{
			...
			try {
			PreparedStatement pstmt1 =conn.prepareStatement("INSERT INTO FIM0300_MANUAL_TABLE(COUNTRY_CODE,CURRENCY_CODE,UNITS,TTB_RATE,TCB_RATE,NTB_RATE,TTS_RATE,NTS_RATE,DEL_FLG,RCRE_USER_ID, RCRE_TIME,REPORT_DATE) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)");
