...
public class BulkUploadSup1100Action 
{
	...
	public String sup1100Upload() throws SQLException, IOException 
	{
		System.out.println("inside xcel fn");
		...
		{
			sql1.append("TRUNCATE  TABLE SUP1100_MANUAL_TABLE");
					          ...
					          .open(is)) 
			{
					  ...
					  for (Sheet s : workbook)
					  {
					    ...
					    for (Row r : s) 
					    {
							...
							if (i1 > 0) {
								...
								String query = "INSERT INTO SUP1100_MANUAL_TABLE(CUST_NAME,UNIQUE_ID,SANC_DATE,TYPE_OF_FACILITY,AMT,PURPOSE,SECURITY_DETAILS,SEC_VALUATION_DT,SEC_VALUE,SEC_VALUER_NAME,NPA_DATE,REALIS_VALUE_OF_SEC,PROVISION,LEGAL_ACTION_DT,BALANCE_ON_FORECLOSURE,SALE_BY_LEVY_DATE,SALE_REF_NO,SECURITY_ENFORCED,ACTUAL_SALE_DATE,AMOUNT_OF_SALE,AMT_ATTRIBUTED_BANK,RECOVERY_COST,ASSET_PURCHASED,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
								...
								stmt.setString(20,dataHolder.get(19));
								stmt.setString(21,dataHolder.get(20));
								stmt.setString(22,dataHolder.get(21));
								stmt.setString(23,dataHolder.get(22));
								stmt.setString(24,"N");
								stmt.setString(25,session.getAttribute("userName").toString());
								stmt.setString(26,df.formatdate(Format.format(date)));
								
								stmt.setString(27,dataHolder.get(23));
								System.out.println(dataHolder.get(23));
	            ...
	            System.out.println("inside the catch");
					...
					if (rs > 0) 
					{
						...
						pstmt.setString(1, "SUP1100_MANUAL");
						...
						status = "success";
						request.setAttribute("Page", "bulkuploadsup1100");
	...
	public String sup1100Upload1() throws SQLException, IOException 
	{
		System.out.println("inside csv fn");
		...
		for (int i = 0; i < fileUpload.length; i++) 
		{
			...
			try {
			PreparedStatement pstmt1 =conn.prepareStatement("INSERT INTO SUP1100_MANUAL_TABLE(CUST_NAME,UNIQUE_ID,SANC_DATE,TYPE_OF_FACILITY,AMT,PURPOSE,SECURITY_DETAILS,SEC_VALUATION_DT,SEC_VALUE,SEC_VALUER_NAME,NPA_DATE,REALIS_VALUE_OF_SEC,PROVISION,LEGAL_ACTION_DT,BALANCE_ON_FORECLOSURE,SALE_BY_LEVY_DATE,SALE_REF_NO,SECURITY_ENFORCED,ACTUAL_SALE_DATE,AMOUNT_OF_SALE,AMT_ATTRIBUTED_BANK,RECOVERY_COST,ASSET_PURCHASED,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE)values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            while((nextLine = reader.readNext()) != null) 
            {
                ...
                pstmt1.setString(20,nextLine[19]);
                pstmt1.setString(21,nextLine[20]);
                pstmt1.setString(22,nextLine[21]);
                pstmt1.setString(23,nextLine[22]);
                pstmt1.setString(24,"N");
                pstmt1.setString(25,session.getAttribute("userName").toString());
                pstmt1.setString(26,df.formatdate(Format.format(date)));
                pstmt1.setString(27,nextLine[23]);
	            ...
	            System.out.println("inside the catch");
					...
					if (rs > 0) 
					{
						...
						if (validstat.equals("Y")) {
							...
							Errormsg = notes.getSubString(1, 20);
