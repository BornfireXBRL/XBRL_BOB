...
public class BulkUploadFim0800Action {
	
	static Logger log = Logger.getLogger(BulkUploadFim0800Action.class);
	...
	public String fim0800Upload() throws SQLException, IOException {
		log.info("xcel fn ");
		...
		if (Integer.parseInt(request.getParameter("cnt")) > 0) {
			...
			sql1.append("TRUNCATE  TABLE FIM0800_MANUAL_TABLE");
		...
		for (int i = 0; i < fileUpload.length; i++) {
				...
				for (Sheet s : workbook) {
					...
					for (Row r : s) 
					{
						...
						if (i1 > 0) {
							...
							log.info(dataHolder);
							String query = "Insert into FIM0800_MANUAL_TABLE(TRADE_DATE,VALUE_DATE,TRAN_TYPE,BILL_CODE,BILL_HOLDER,COUNTER_PARTY_CODE,AMOUNT,YIELD,BILL_PRICE,MATURITY_DATE,BILL_BAND,ISIN,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
							...
							stmt.setString(16, dataHolder.get(12));
			...
			if (rs > 0) {
				...
				pstmt.setString(1, "FIM0800_MANUAL");
				...
				status = "success";
				request.setAttribute("Page", "bulkuploadfim800");
	...
	public String fim0800Upload1() throws SQLException, IOException {
		...
		for (int i = 0; i < fileUpload.length; i++) {
			...
			try {
				PreparedStatement pstmt1 = conn.prepareStatement(
						"Insert into FIM0800_MANUAL_TABLE(TRADE_DATE,VALUE_DATE,TRAN_TYPE,BILL_CODE,BILL_HOLDER,COUNTER_PARTY_CODE,AMOUNT,YIELD,BILL_PRICE,MATURITY_DATE,BILL_BAND,ISIN,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
				...
				while ((nextLine = reader.readNext()) != null) {
					...
					if (i1 > 0) {
						...
						pstmt1.setString(16, nextLine[12]);
