...
public class BulkUploadSup1300Action 
{
	...
	public String sup1300Upload() throws SQLException, IOException 
	{
		...
		if (Integer.parseInt(request.getParameter("cnt")) > 0) {
			...
			sql1.append("TRUNCATE  TABLE SUP1300_MANUAL_TABLE");
		...
		for (int i = 0; i < fileUpload.length; i++) {
				...
				for (Sheet s : workbook) {
					...
					for (Row r : s) 
					{
						...
						if (i1 > 0) {
							...
							String query = "Insert into SUP1300_MANUAL_TABLE(INSTANCE_CODE,INDIVIDUALS,CORPORATES,OTHERS,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE) values(?,?,?,?,?,?,?,?)";
							...
							stmt.setString(4, dataHolder.get(4));
							
							stmt.setString(5, "N");
							stmt.setString(6, session.getAttribute("userName").toString());
							stmt.setString(7, df.formatdate(Format.format(date)));
							stmt.setString(8, dataHolder.get(5));
			...
			if (rs > 0) {
				...
				pstmt.setString(1, "SUP1300_MANUAL");
				...
				status = "success";
				request.setAttribute("Page", "bulkuploadsup1300");
	...
	public String sup1300Upload1() throws SQLException, IOException 
	{
		...
		for (int i = 0; i < fileUpload.length; i++) {
			...
			try {
				PreparedStatement pstmt1 = conn.prepareStatement(
						"Insert into SUP1300_MANUAL_TABLE(INSTANCE_CODE,INDIVIDUALS,CORPORATES,OTHERS,DEL_FLG,RCRE_USER_ID,RCRE_TIME,REPORT_DATE) values(?,?,?,?,?,?,?,?)");
				...
				while ((nextLine = reader.readNext()) != null) {
					...
					if (i1 > 0) {
						...
						pstmt1.setString(4, nextLine[4]);
						
						pstmt1.setString(5, "N");
						pstmt1.setString(6, session.getAttribute("userName").toString());
						pstmt1.setString(7, df.formatdate(Format.format(date)));
						pstmt1.setString(8, nextLine[5]);
