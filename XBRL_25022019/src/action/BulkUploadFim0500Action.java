...
public class BulkUploadFim0500Action 
{
	static Logger log = Logger.getLogger(BulkUploadFim0500Action.class);
	...
	public String fim0500Upload() throws SQLException, IOException 
	{
		log.info("xcel fn ");
		...
		{
			sql1.append("TRUNCATE  TABLE FIM0500_MANUAL_TABLE");
					          ...
					          .open(is)) 
			{
					  ...
					  for (Sheet s : workbook)
					  {
					    ...
					    for (Row r : s) 
					    {
							...
							if (i1 > 0) {
								...
								log.info(dataHolder);
								String query = "INSERT INTO FIM0500_MANUAL_TABLE(TRAN_ID,TRAN_TYPE,FCY_CRNCY,TRAN_AMT,INSTITUTE_ID,INSTITUTE_NAME,SECTOR,SPOT_RATE,FWD_RATE,FWD_DATE,REPORT_DATE,REPORTING_TIME,DEL_FLG,RCRE_USER_ID,RCRE_TIME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	            ...
	            log.info("inside the catch");
					...
					if (rs > 0) 
					{
						...
						pstmt.setString(1, "FIM0500_MANUAL");
						...
						status = "success";
						request.setAttribute("Page", "bulkuploadfim500");
	...
	public String fim0500Upload1() throws SQLException, IOException 
	{
		...
		for (int i = 0; i < fileUpload.length; i++) 
		{
			...
			try {
			PreparedStatement pstmt1 =conn.prepareStatement("INSERT INTO FIM0500_MANUAL_TABLE(TRAN_ID,TRAN_TYPE,FCY_CRNCY,TRAN_AMT,INSTITUTE_ID,INSTITUTE_NAME,SECTOR,SPOT_RATE,FWD_RATE,FWD_DATE,REPORT_DATE,REPORTING_TIME,DEL_FLG,RCRE_USER_ID,RCRE_TIME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
