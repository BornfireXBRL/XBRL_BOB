...
public class BulkUploadFim0200Action {
	static Logger log = Logger.getLogger(BulkUploadFim0200Action.class);
	...
	public String fim0200Upload() throws SQLException, IOException {
		...
		if (Integer.parseInt(request.getParameter("cnt")) > 0) {
			sql1.append("TRUNCATE  TABLE FIM0200_MANUAL_TABLE");
		...
		for (int i = 0; i < fileUpload.length; i++) {
				...
				for (Sheet s : workbook) {
					...
					for (Row r : s) {
						...
						if (i1 > 0) {
							...
							log.info(dataHolder);
							String query = "Insert into FIM0200_MANUAL_TABLE(INSTITUTE_NAME,INSTITUTE_CODE,PUR_CRNCY,PUR_AMT,SALE_CRNCY,SALE_AMT,RATE,VALUE_DATE,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			if (rs > 0) {
				...
				pstmt.setString(1, "FIM0200_MANUAL");
				...
				status = "success";
				request.setAttribute("Page", "bulkuploadfim200");
	...
	public String fim0200Upload1() throws SQLException, IOException {
		...
		for (int i = 0; i < fileUpload.length; i++) {
			...
			try {
				PreparedStatement pstmt1 = conn.prepareStatement(
						"Insert into FIM0200_MANUAL_TABLE(INSTITUTE_NAME,INSTITUTE_CODE,PUR_CRNCY,PUR_AMT,SALE_CRNCY,SALE_AMT,RATE,VALUE_DATE,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?)");
