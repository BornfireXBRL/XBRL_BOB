...
public class BulkUploadSup1400Action 

{
	...
	public String sup1400Upload() throws SQLException, IOException 
	{
		...
		if (Integer.parseInt(request.getParameter("cnt")) > 0) {
			...
			sql1.append("TRUNCATE  TABLE SUP1400_MANUAL_TABLE");
		...
		for (int i = 0; i < fileUpload.length; i++) {
				...
				for (Sheet s : workbook) {
					...
					for (Row r : s) 
					{
						...
						if (i1 > 0) {
							...
							String query = "Insert into SUP1400_MANUAL_TABLE(COMPLAINT_RCVD_DATE,NAME_OF_COMPLAINANT,ADDRESS_OF_COMPLAINANT,COMPLAINT_NATURE,COMPLAINT_NATURE_DET,RESOLVE_DATE,NATURE_OF_RESOLUTION,UNRESOLVED_DET,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?)";
							...
							stmt.setString(10, "N");
							stmt.setString(11, session.getAttribute("userName").toString());
							stmt.setString(12, df.formatdate(Format.format(date)));
			...
			if (rs > 0) {
				...
				pstmt.setString(1, "SUP1400_MANUAL");
				...
				status = "success";
				request.setAttribute("Page", "bulkuploadsup1400");
	...
	public String sup1400Upload1() throws SQLException, IOException 
	{
		...
		for (int i = 0; i < fileUpload.length; i++) {
			...
			try {
				PreparedStatement pstmt1 = conn.prepareStatement(
						"Insert into SUP1400_MANUAL_TABLE(COMPLAINT_RCVD_DATE,NAME_OF_COMPLAINANT,ADDRESS_OF_COMPLAINANT,COMPLAINT_NATURE,COMPLAINT_NATURE_DET,RESOLVE_DATE,NATURE_OF_RESOLUTION,UNRESOLVED_DET,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?)");
				...
				while ((nextLine = reader.readNext()) != null) {
					...
					if (i1 > 0) {
						...
						pstmt1.setString(11, session.getAttribute("userName").toString());
						pstmt1.setString(12, df.formatdate(Format.format(date)));
