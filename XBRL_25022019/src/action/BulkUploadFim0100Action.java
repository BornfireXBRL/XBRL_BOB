...
public class BulkUploadFim0100Action {
	static Logger log = Logger.getLogger(BulkUploadFim0100Action.class);
	...
	public String fim0100Upload() throws SQLException, IOException {
		...
		if (Integer.parseInt(request.getParameter("cnt")) > 0) {
			sql1.append("TRUNCATE  TABLE FIM0100_MANUAL_TABLE");
		...
		for (int i = 0; i < fileUpload.length; i++) {
				...
				for (Sheet s : workbook) {
					...
					for (Row r : s) {
						...
						if (i1 > 0) {
							...
							log.info(dataHolder);
							String query = "Insert into FIM0100_MANUAL_TABLE(INSTITUTE_NAME,TRAN_NATURE,INSTITUTE_CODE,TRAN_CRNCY,TRAN_TYPE,TRAN_AMT,INT_START_DATE,INT_END_DATE,INT_RATE,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
							...
							stmt.setString(11, "N");
							stmt.setString(12, session.getAttribute("userName").toString());
							stmt.setString(13, df.formatdate(Format.format(date)));
			...
			if (rs > 0) {
				...
				pstmt.setString(1, "FIM0100_MANUAL");
				...
				status = "success";
				request.setAttribute("Page", "bulkuploadfim100");
	...
	public String fim0100Upload1() throws SQLException, IOException {
		...
		for (int i = 0; i < fileUpload.length; i++) {
			...
			try {
				PreparedStatement pstmt1 = conn.prepareStatement(
						"Insert into FIM0100_MANUAL_TABLE(INSTITUTE_NAME,TRAN_NATURE,INSTITUTE_CODE,TRAN_CRNCY,TRAN_TYPE,TRAN_AMT,INT_START_DATE,INT_END_DATE,INT_RATE,REPORT_DATE,DEL_FLG,RCRE_USER_ID,RCRE_TIME) values(?,?,?,?,?,?,?,?,?,?,?,?,?)");
				...
				while ((nextLine = reader.readNext()) != null) {
					...
					if (i1 > 0) {
						...
						pstmt1.setString(12, session.getAttribute("userName").toString());
						pstmt1.setString(13, df.formatdate(Format.format(date)));
